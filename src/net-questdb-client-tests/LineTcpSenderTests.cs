...
public class LineTcpSenderTests
{
    ...
    public async Task Authenticate()
    {
        ...
        using var ls = new LineSender($"tcp::addr={_host}:{_port};username=testUser1;token=NgdiOWDoQNUP18WOnb1xkkEG5TzPYMda5SiUOvT1K0U=;");
    ...
    public async Task AuthFailWrongKid()
    {
        ...
        Assert.That(
            ...
            ,
            Throws.TypeOf<AggregateException>().With.InnerException.TypeOf<IngressError>().With.Message.Contains("Authentication failed")
            );
    }
    ...
    public async Task AuthFailBadKey()
    {
        ...
        Assert.That(
            ...
            Throws.TypeOf<AggregateException>().With.InnerException.TypeOf<IngressError>().With.Message.Contains("Could not write data to server.")
        );
    }
    ...
    public async Task WithTls()
    {
        ...
        using var sender = new LineSender($"tcps::addr={_host}:{_port};username=testUser1;token=NgdiOWDoQNUP18WOnb1xkkEG5TzPYMda5SiUOvT1K0U=;tls_verify=unsafe_off;");
