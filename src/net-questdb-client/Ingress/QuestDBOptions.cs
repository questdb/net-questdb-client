...
public class QuestDBOptions
{
    ...
    private readonly string? _auth_timeout;
    private readonly string? _auto_flush;
    private readonly string? _auto_flush_bytes;
    private readonly string? _auto_flush_interval;
    private readonly string? _auto_flush_rows;
    private string? _bind_interface;
    private readonly string? _init_buf_size;
    private readonly string? _max_buf_size;
    private readonly string? _max_name_len;
    private readonly string? _own_socket;

    private string? _password;

    // backing/raw fields
    private readonly string _protocol;
    private readonly string? _request_min_throughput;
    private readonly string? _request_timeout;
    private readonly string? _retry_timeout;
    private string? _tls_ca;
    private string? _tls_roots;
    private string? _tls_roots_password;
    private readonly string? _tls_verify;
    ...
    public QuestDBOptions(string confStr)
    {
        ...
        if (_auto_flush_bytes is null)
            _auto_flush_rows ??= _protocol == ProtocolType.http.ToString() || _protocol == ProtocolType.https.ToString()
                ? "75000"
                : "600";

        if (_auto_flush_rows is null) _auto_flush_bytes ??= (1024 * 100).ToString();
        ...
        auto_flush = Enum.Parse<AutoFlushType>(_auto_flush, false);
        ...
        tls_verify = Enum.Parse<TlsVerifyType>(_tls_verify, false);
    ...
    [JsonIgnore] public bool OwnSocket { get; init; }

    public string? token_x { get; }

    public string? token_y { get; }
