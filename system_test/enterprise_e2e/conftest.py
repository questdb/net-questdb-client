"""
Pytest root config for net-questdb-client Enterprise e2e tests.

Registers the Enterprise shared_fixtures plugin (server_factory,
scenario_dir, obj_store, etc.) and adds a ``net_sidecar`` fixture
that builds and launches the .NET sidecar console app.
...
SIDECAR_DLL = SIDECAR_DIR / "bin" / "Release" / "net10.0" / "net-questdb-client-e2e-sidecar.dll"


class NetSidecarError(RuntimeError):
    pass


@dataclass
class NetSidecarStats:
    acked: int
    sent: int
    acks: int
    reconn_attempts: int
...
class NetSidecar:
    ...
    def start(self, *, ready_timeout: float = 60.0) -> None:
        ...
        if not dll.exists():
            raise FileNotFoundError(
                f"sidecar DLL not found at {dll}; "
                f"run 'dotnet build -c Release' in {SIDECAR_DIR} first"
            )
        ...
        LOG.info("starting .NET sidecar %s", self.name)
    ...
    # ---- protocol verbs (same as the Java sidecar) ----
    ...
    def stats(self) -> NetSidecarStats:
        self._send("STATS")
        reply = self._expect_ok()
        kv = dict(p.split("=", 1) for p in reply if "=" in p)
        return NetSidecarStats(
    ...
    def _expect_ok(self) -> list[str]:
        ...
        if line.startswith("ERR"):
            raise NetSidecarError(line[len("ERR "):])
...
def net_sidecar(log_dir: Path) -> Iterator[NetSidecar]:
    s = NetSidecar(log_dir=log_dir, name="net-sidecar")
    s.start()
    try:
        yield s
    finally:
